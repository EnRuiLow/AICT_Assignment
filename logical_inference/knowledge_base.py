...
class MRTKnowledgeBase:
    ...
    The knowledge base is initialized with 17 rules covering the LTA's
    ...
    def _initialize_rules(self):
        """
        Initialize the 17 operational rules.
        
        Rules are organized by category:
        - Transfer rules (R1, R9)
        - Integration work rules (R2, R10)
        - Network mode rules (R3, R4, R7, R11, R12)
        - Mode exclusivity rules (R13, R14)
        - Service status rules (R5, R8)
        - Station closure rules (R6, R15)
        - Additional rules (R16, R17)
        """
        ...
        # ====================================================================
        # NETWORK MODE EXCLUSIVITY RULES (NEW - CRITICAL FIX)
        # ====================================================================
        
        self.rules.append(LogicRule(
            rule_id="R13",
            description="If network is in TODAY mode, then it is NOT in FUTURE mode",
            antecedent=["Network_Mode_Today"],
            consequent="Network_Mode_Not_Future"
        ))
        
        self.rules.append(LogicRule(
            rule_id="R14",
            description="If network is in FUTURE mode, then it is NOT in TODAY mode",
            antecedent=["Network_Mode_Future"],
            consequent="Network_Mode_Not_Today"
        ))
        ...
        self.rules.append(LogicRule(
            rule_id="R15",
            description="If Tanah Merah station is closed, then TEL-EWL transfer is NOT available",
            antecedent=["Station_Closed_TanahMerah"],
            consequent="Transfer_Unavailable_TEL_EWL"
        ))
        
        # ====================================================================
        # ADDITIONAL TRANSFER AND SERVICE RULES
        # ====================================================================
        
        self.rules.append(LogicRule(
            rule_id="R16",
            description="If TEL line has disrupted service, then TEL-dependent transfers may be affected",
            antecedent=["Service_Status_Disrupted_TEL"],
            consequent="Transfer_Risk_TEL"
        ))
        
        self.rules.append(LogicRule(
            rule_id="R17",
            description="If old EWL airport branch is active, then network must be in TODAY mode",
            antecedent=["Line_Active_EWL_Airport"],
            consequent="Network_Mode_Today"
        ))
